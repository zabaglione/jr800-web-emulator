...
「未対応I/Oで停止しない」は初期状態で有効です。未実装I/Oへの読み取りは`$FF`を返し、書き込みは破棄して実行を続けます。
設定はROM読み込み時に適用され、無視した回数は開発者向けデバッガーで確認できます。未対応アクセスで停止させる場合は、設定を外してROMを読み込み直します。
...
「液晶の見た目」では、コントラスト、背景の明るさ、色合い、ドットの隙間、消灯ドットの濃さを調整できます。
見た目の設定はブラウザーに保存されます。ROMやRAMの内容は保存しません。
LCD左右の三角印は既知のBASIC表示要求に連動し、電池寿命の印は常に表示します。実機の電気的な点灯条件を確定するものではありません。
