...
現在は開発中の実験版です。
実装および動作確認は、JR-HuBASIC 1.0を基準に行っています。
...
再ビルド後はページを再読み込みしてください。配信ファイルはビルド内容に応じたURLで一式を読み込み、画面とWASMの新旧混在を防ぎます。
...
公開用リポジトリ向けの配信設定を用意しています。公開後の設定は[GitHub Pagesの手順](docs/user/github-pages.md)を参照してください。
読み込みに成功したROMとBASICの起動設定は、このブラウザー内に保存します。再訪時には保存済みROMの名前が表示され、「BASICの起動」を押したときにエミュレーターへ読み込みます。自動起動はしません。
プログラムと作業用RAMはページを閉じると消えます。ROMを含め、選択したファイルをサーバーへ送信する処理はありません。
...
「ブラウザーの現在日時でBASICを起動する」は、端末のローカル日時、曜日、RTCのうるう年カウンターを起動時に一度だけ設定します。
対象は2000〜2099年です。
オフにするとROM本来の初期値を使い、`Load only`では日時を設定しません。
ROMファイルは変更しません。
時計はエミュレーターの実行時間に合わせて進み、PauseやWebの電源OFF中は止まります。
再開時にPCの時刻へ合わせ直すことはありません。

`DATE$`の年は従来どおり2桁で、`DAY$`は独立しています。
起動後に`DATE$`で別の年へ変更しても、曜日とうるう年カウンターは自動で再設定されません。
現在日時に戻すには「ブラウザーの現在日時でBASICを起動する」をオンにしてBASICを起動し直してください。
`WAKE`によるアラーム、電源制御、復帰は未対応です。
日時の扱いと未対応機能は[対応範囲](docs/user/compatibility.md)を参照してください。
...
3. `Program .WAV / .J8A`で配布された`.wav`を選び、`Load and run program`を押します。
...
BASICのWAVも機械語と同じプログラム欄から読み込めます。通常の`SAVE`と`SAVE ...,A`を自動判定し、J8A v1への保存にも対応します。手順と制限は[BASICプログラムWAVの利用ガイド](docs/user/basic-program-wav.md)を参照してください。形式エラーになるファイルは元のWAVから作り直してください。

BASICの入力待ちで一時停止し、プログラム欄からファイルを選びます。
「読み込む」は内容の確認用、「読み込んで実行する」は読込後にRUNする操作です。

## エミュレーターからJ8A・WAVへ保存する

BASICで`SAVE "MyBasic"`を実行すると、完了後に「データをファイルへ保存」へ項目が追加されます。
「J8Aをダウンロード」「WAVをダウンロード」から、`MyBasic.j8a`／`MyBasic.wav`を保存できます。
`SAVE ...,A`と`MSAVE`も同じ操作で保存でき、ファイル名は命令で指定した名前を使います。
保存一覧はページ内で最大32件保持します。ページを閉じる前にダウンロードしてください。
手順と実機確認の範囲は[保存ガイド](docs/user/program-save-export.md)を参照してください。
...
| `Program .WAV / .J8A` | SAVE／MSAVEのWAV、または変換済みJR8APPを選ぶ欄です。JR-800のROM読込後に使えます。 |
| `Load program` | 種類を判定して読み込み、停止状態を保ちます。 |
| `Load and run program` | 読込後にBASICのRUN、または機械語の指定アドレスから実行します。 |
...
ROMはブラウザー内のIndexedDBに保存します。「ブラウザー内のROMを削除」で消去できます。
同じURLを同じブラウザーで開いた場合に利用でき、別のブラウザーや公開URLへは引き継ぎません。ブラウザーのサイトデータを消去するとROMも消えます。RAMとプログラムはセッション内だけで扱い、ファイルのアップロードは行いません。

BASICは標準16KB RAMで起動します。8KB拡張RAMを使う場合だけ、「実験用の詳細設定」のRAM欄で「8KB拡張RAMを接続（オプション）」を選び、BASICを起動してください。変更は次のBASIC起動から反映されます。
拡張なしの起動では、未接続領域への読込を`$FF`、書込を破棄する実験上の扱いを使います。実機の未接続時の値は未確認です。

LCD左右のインジケーターは、現時点では候補RAMの生値を表示する診断情報です。
値が0か非0かだけで、実機の消灯・点灯を確定することはできません。
`OFF`はROM・RAMなどの状態を保持して実行を停止し、`ON`はその続きから再開します。
開発者用デバッガーの「再起動（RAMを初期化）」は、確認後に作業用RAMと周辺機器を起動時の状態へ戻します。ROMと完了済みの保存データ一覧は残ります。ブラウザー時刻を使って起動していた場合は時刻も再設定します。
BASICの連続実行は、内部のスリープ用サイクル上限で自動停止しません。
実機の電源回路を再現する操作ではありません。
...
`CTRL` + `9`でCAP.L、`CTRL` + `0`でGRAPH、`CTRL` + `^`でカナを切り替えると、仮想キーボードの表示もBASICの状態に追従します。
CAP.L中は英大文字を表示し、`SHIFT`で小文字に反転します。カナでは`SHIFT`で小文字や対応する記号を表示します。
文字や図形の字形は、読み込んだROMから取得します。`CTRL` + `8`による機能モード・制御モードの切替もキー表示へ反映します。
キーボードの確認済みの範囲は[対応範囲](docs/user/compatibility.md)を参照してください。
...
拡張RAMが必要な場合は`--expansion-ram-initial 0`を追加してください。
...
- `Load and run program`が使えない場合は、先にJR8ROMを読み込み、実行中なら`Pause`を押してください。
