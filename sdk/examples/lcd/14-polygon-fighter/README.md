...
最初の版と全64方向の画素・陰影・面の順序を保ち、平均処理時間を約57%削減しました。

| 一周64フレームでの測定 | 最初の版 | 最適化後 |
| --- | ---: | ---: |
| 平均Eサイクル / フレーム | 250,842 | 108,027 |
| 公称クロックから換算した平均fps | 4.90 | 11.37 |
| 一周の時間 | 13.07秒 | 5.63秒 |
| 平均LCDデータ転送 / フレーム | 1,536バイト | 427.81バイト |
| コードと固定データ | 2,132バイト | 3,779バイト |

- 毎フレームの固定待ちを外し、待ちループは一時停止中だけに限定しています。
- 前フレームの機体が占めた範囲だけを消去し、新旧の機体範囲を合わせた部分だけLCDへ転送します。見出しと操作説明は初回に転送します。
...
- 三角形の頂点をY順に並べ、上下端を結ぶ辺でその三角形が使う行だけ初期化します。辺の傾きの商・余りを最初に求め、各行は加算と最大1回の補正で進めます。
- 行を移る際のLCDアドレスとビットマスクを逐次更新し、黒／白の処理を分けて8列ずつ塗ります。1〜2ドット幅の行は輪郭だけを描きます。

変換済みの頂点や回転画像を保存する方式ではなく、座標変換・陰影・隠面処理は毎フレームCPUで実行します。
コード量を増やして処理を短縮していますが、描画用RAMは引き続き1画面分です。
...
| `spans.s` | 黒／白の組み合わせ別に8列ずつ処理する塗りつぶし |
...
`make test`は生成元との一致も検査します。生成時に作るのは固定の形状・法線・材質フラグ・正弦表だけです。

最終ビルドの配置量は、コードと固定データ3,779バイト、描画用RAM1,536バイト、状態347バイト、BASIC復帰処理76バイト、退避領域132バイトの**合計5,870バイト**です。
別途CPU内RAMの`$80–$C7`（表示・転送18＋描画54＝72バイト）とスタックを使います。SPは`$5FFF`で、上端512バイトをスタック用に空けています。
...
頂点・面の作業領域も20頂点・23面分です。形状や拡大率を変える場合は、作業領域と全64方向での表示範囲を確認してください。検査には最初の版の全64フレームのSHA-256も固定しているため、意図的に形状を変えた場合は、新しい表示を確認したうえでその参照値も更新します。

## 検証範囲

- **ビルド・静的確認**: 付属SDKでアセンブル・リンク、モデルの再生成一致、配置量を確認。
- **WASM実行**: 全64方向の座標、面の前後判定、陰影、奥行き順、全ピクセルの塗りつぶしを参照計算と比較。最初の版の全64フレームともSHA-256で照合。LCD転送、スタック、コードと周辺RAMの保護、一周後の完全一致、押下／押し続け／解放／再開も検査。
- **短いキー入力**: 連続描画中の21通りの開始時刻で49,152 Eサイクルの5入力を与え、一時停止と再開を確認。全64方向での最大キー確認間隔は15,708 Eサイクル（公称約12.8ms）で、最小保持時間より短いことも検査。
...
一周分の描画は1フレーム98,518〜118,614 Eサイクル、平均108,026.703125 Eサイクルでした。
公称1.2288 MHzで換算すると約10.36〜12.47 fps、平均11.37fps、一周約5.63秒です。ブラウザーの実時間や実機のLCD応答を保証する値ではありません。
...
検査は平均110,000・最大125,000 Eサイクル未満、通常フレームのLCD転送530バイト以下も要求します。
