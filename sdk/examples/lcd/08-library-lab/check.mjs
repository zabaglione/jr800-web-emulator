...
function frame() {
    ...
    const stop = machine.runTo(symbols.frame_ready, 2_000_000);
...
try {
    ...
    frame();
    assert.equal(sample, "08-library-lab");
    const timing = {};
    const timedFrame = name => {
        const before = Number(machine.state().cycleCount);
        const panel = frame();
        timing[name] = Number(machine.state().cycleCount) - before;
        return panel;
    };
    if (mode === "test") {
        const terrain = [...machine.memory(symbols.framebuffer + 768, 192)];
        assert.equal(read("position"), 92);
        machine.setKeyboardKeyState("keypad-4", true);
        timedFrame("moveLeft");
        assert.equal(read("position"), 91);
        assert.deepEqual([...machine.memory(symbols.key_state, 3)], [16,0,0]);
        assert.deepEqual([...machine.memory(symbols.framebuffer + 768, 192)], [...terrain.slice(1),0]);
        frame();
        assert.deepEqual([...machine.memory(symbols.key_state, 3)], [16,0,0]);
        machine.setKeyboardKeyState("keypad-4", false);
        timedFrame("idle");
        assert.deepEqual([...machine.memory(symbols.key_state, 3)], [0,0,0]);
        machine.setKeyboardKeyState("keypad-6", true);
        timedFrame("moveRight");
        assert.equal(read("position"), 91);
        machine.setKeyboardKeyState("keypad-6", false);
        frame();
        for (const [key, label] of [["keypad-5","effect"],["keypad-7","melody"]]) {
            machine.setKeyboardKeyState(key,true);
            timedFrame(label);
            assert.equal(read("sound_count"), 0);
            // A held key must not start the blocking sequence again.
            timedFrame(label + "Held");
            assert.ok(timing[label] > timing[label + "Held"] + 50000);
            machine.setKeyboardKeyState(key,false);frame();
        }
    }
    const paths = [];
    ...
    const result = {sample, mode, model: "JR-800 WASM",
        ...
        frames, timing, cycles: machine.state().cycleCount, firstFrameSha256:
            createHash("sha256").update(firstPanel.dots).digest("hex"), passed: true};
    await writeFile(resolve(outputDirectory, mode === "test" ? "verification.json" : `${mode}.json`), JSON.stringify(result, null, 2) + "\n");
