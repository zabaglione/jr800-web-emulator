...
export async function checkOrchard(g){
 const levels=JSON.parse(await readFile(new URL('./levels.json',import.meta.url))),solutions=JSON.parse(await readFile(new URL('./solutions.json',import.meta.url)));await g.start();await g.save('gameplay-1');let second=false,third=false,beans=false,berries=false;
 function key(k){let old=state(g).p;if(k==='next'||k==='swap'){while(!g.read('orchard_button'))g.tap('down');const choice=k==='next'?1:2;if(g.read('orchard_button')!==choice)g.tap('right');}const expected=action(state(g),k,levels[g.read('stage')]);g.tap('space');if(g.read('phase')===2&&g.read('orchard_button')){g.tap('up');move(old);expected.p=old;}assert.deepEqual(state(g),expected,'Main-screen actions preserve crop rules, money and the daily work budget');}
 ...
 const frozen=state(g);for(let i=0;i<25;i++){const blink=g.read('cursor_blink_mask');g.frame();if(blink===g.read('cursor_blink_mask'))assert.equal(g.word('dirty_bytes'),0);else assert.ok(g.word('dirty_bytes')<=128,'Only the focus marker is transferred');}assert.deepEqual(state(g),frozen);key('space');key('space');assert.equal(g.read('orchard_actions'),2,'Watering an already watered seed uses no extra action');for(let i=0;i<5;i++)key('next');assert.equal(g.read('orchard_crops',18)[0],0,'Two dry days after the rainy recovery wither the crop');while(g.read('phase')===2)key('next');assert.equal(g.read('phase'),5);g.tap('space');
