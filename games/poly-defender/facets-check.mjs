...
const poses=Object.values(geometry).flat();
...
const lines=[[0,8,191,8],[191,55,0,55],[40,0,40,63],[100,63,100,0],
    ...
    [16,16,152,16],[16,48,160,48],[0,8,191,8],[0,55,191,55]];
function drawLine(pixels,[x0,y0,x1,y1],dashed=false){
    ...
    for(let i=0;i<=steps;i++){
        ...
        if(y>=8 && y<=55 && (!dashed || (x&4)===0))pixels[y*192+x]=2;
    }
}
try {
    for(let frame=0;frame<180;frame++){
        ...
        else if(group===11)drawLine(expected,lines[i],i>=16);
        else if(group===12 && i===1){draw(expected,geometry.ship[0],93,33);draw(expected,geometry.rotor[0],90,33);}
        else if(group===13 || group===14)draw(expected,geometry.aim[0],152,group===13?16:48,2);
        else if(group===15)draw(expected,geometry.core[0],152,32,2);
    ...
    await f.finish({oracleCases:180,geometryPoses:21,lineCases:20,verticalClipping:true,doubleScale:true,opaqueOverlap:true,erasedPreviousBounds:true});
} finally {f.close();}
