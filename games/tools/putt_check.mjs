...
export async function checkPutt(g){
 const levels=JSON.parse(await readFile(new URL('../wind-putt/levels.json',import.meta.url))),solutions=JSON.parse(await readFile(new URL('../wind-putt/solutions.json',import.meta.url)));let ticks=0,bounces=0;await g.start();
 ...
 const idle=state(g);for(let n=0;n<20;n++){g.frame();assert.equal(g.word('dirty_bytes'),0);}assert.deepEqual(state(g),idle);await g.save('gameplay-1');
