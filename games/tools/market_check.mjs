...
export async function checkMarket(g){
 ...
 function open(dest){g.menu(1);while(g.read('market_destination')!==dest)g.tap('down');const s=state(g),level=levels[g.read('stage')],expected=[];for(let port=0;port<4;port++)for(let k=0;k<3;k++)expected.push(price(port,s.day+trip(port,s.port),level,k));assert.deepEqual(g.read('market_quotes',12),expected,'Port list quotes arrival-day prices, including the free current-port return');return s;}
 ...
 const frozen=state(g);for(let n=0;n<25;n++){g.frame();assert.equal(g.word('dirty_bytes'),0);}assert.deepEqual(state(g),frozen);
