...
export async function checkDispatch(g){
 const schedules=JSON.parse(await readFile(new URL('../rail-dispatch/levels.json',import.meta.url))),network=JSON.parse(await readFile(new URL('../rail-dispatch/network.json',import.meta.url)));await g.start();await g.save('gameplay-1');let worlds=0,second=false,third=false;
