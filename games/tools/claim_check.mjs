...
export async function checkClaim(g){
 const arenas=JSON.parse(await readFile(new URL('../grid-claim/levels.json',import.meta.url)));await g.start();await g.save('gameplay-1');g.frame();assert.equal(g.word('dirty_bytes'),0);
 ...
 for(let stage=0;stage<3;stage++){
  ...
  while(g.read('phase')===2){
   assert.equal(g.read('claim_mode'),0);const arena=g.read('claim_arena');seen.add(arena);const expected=[...arenas[arena]];expected[50]=1;expected[61]=2;assert.deepEqual(g.read('board',112),expected);
