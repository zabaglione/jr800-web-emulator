...
function fresh(level){return {lanes:structuredClone(level.lanes),periods:[...level.periods],timers:[...level.periods],homes:Array(5).fill(0),p:90,running:0,lives:5,time:100,score:0,phase:2};}
function spawn(s){s.p=90;s.time=100;s.running=0;}
function die(s){if(--s.lives)spawn(s);else{s.running=0;s.phase=5;}}
function check(s){const x=s.p%14,y=Math.floor(s.p/14);if(y===0){const i=homes.indexOf(x);if(i<0||s.homes[i])return die(s);s.homes[i]=1;s.score+=100+s.time;if(s.homes.every(Boolean)){s.phase=4;s.running=0;}else spawn(s);return;}
...
function frame(g,s,key){const before=g.read('river_clock'),resume=g.read('resume_pending'),starting=key==='space'&&!s.running;if(starting)s.running=1;
 if(s.running&&keys.includes(key)){const n=neighbor(s.p,keys.indexOf(key));if(n!==255){s.p=n;check(s);}}
 g.frame();if(!resume&&!starting&&g.read('river_clock')!==before)world(s);inspect(g,s);return !resume&&!starting&&g.read('river_clock')!==before;
}
...
export async function checkRiver(g){const levels=JSON.parse(await readFile(new URL('../river-hop/levels.json',import.meta.url)));await g.start();let s=fresh(levels[0]);await g.save('gameplay-1');g.frame();assert.equal(g.word('dirty_bytes'),0);inspect(g,s);
 // Standing on the starting bank until time expires exercises all five lives and retry.
 while(s.phase===2){if(!s.running)pulse(g,s,'space');else frame(g,s);}assert.equal(s.phase,5);g.tap('space');let second=false,third=false,carried=false,paused=false;
 for(let stage=0;stage<12;stage++){
  if(stage)g.tap('space');s=fresh(levels[stage]);inspect(g,s);
  for(let home=0;home<5;home++){
   pulse(g,s,'space');
   for(let actions=0;s.running;actions++){
    assert.ok(actions<100);const d=plan(s)[0];if(d<4)pulse(g,s,keys[d]);if(!s.running)break;
    ...
    // Wait for one environmental tick, preserving short released direction inputs.
    for(let f=0;f<80;f++){const p=s.p;if(frame(g,s)){if(p!==s.p&&Math.floor(p/14)<3&&Math.floor(p/14)>0)carried=true;break;}assert.ok(f<79);}
    if(stage===4&&!second&&Math.floor(s.p/14)<3&&s.running){await g.save('gameplay-2');second=true;}
   }
   assert.equal(s.lives,5,`Safe crossing ${stage+1}/${home+1}`);assert.equal(s.homes.filter(Boolean).length,home+1);
   if(stage===11&&home===3){await g.save('gameplay-3');third=true;}
  }
  assert.equal(s.phase,4);console.log(JSON.stringify({stage:stage+1,score:s.score}));
 }assert.ok(second&&third&&carried&&paused);
}
