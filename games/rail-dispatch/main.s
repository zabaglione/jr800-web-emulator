...
game_render:
    JSR paint_board
    CLR dispatch_hud_force
    TST resume_pending
    BEQ dispatch_values
    INC dispatch_hud_force
    LDX #game_name
    CLRA
    CLRB
    JSR paint_text
    ...
    INCA
    JSR paint_number
    LDX #dispatch_next_label
    LDAA #138
    LDAB #1
    JSR paint_text
    LDX #dispatch_done_label
    LDAA #138
    LDAB #3
    JSR paint_text
    LDX #dispatch_left_label
    LDAA #138
    LDAB #5
    JSR paint_text
    LDX #dispatch_exit_label
    LDAA #138
    LDAB #7
    JSR paint_text
dispatch_values:
dispatch_time_check:
    TST dispatch_hud_force
    BNE dispatch_time_value
    LDAA dispatch_time
    CMPA dispatch_old_time
    BEQ dispatch_done_check
dispatch_time_value:
    LDAA dispatch_time
    STAA dispatch_old_time
    LDAA #90
    STAA paint_x
    LDAA #0
    STAA paint_band
...
dispatch_done_check:
    TST dispatch_hud_force
    BNE dispatch_done_value
    LDAA dispatch_done
    CMPA dispatch_old_done
    BEQ dispatch_running_check
dispatch_done_value:
    LDAA dispatch_done
    STAA dispatch_old_done
    LDAA #138
    STAA paint_x
    LDAA #4
    STAA paint_band
    ...
    SUBA dispatch_done
    JSR paint_number
dispatch_running_check:
    TST dispatch_hud_force
    BNE dispatch_running_value
    LDAA dispatch_running
    CMPA dispatch_old_running
    BEQ dispatch_switch_check
dispatch_running_value:
    LDAA dispatch_running
    STAA dispatch_old_running
    LDX #dispatch_set_label
    TSTA
    BEQ dispatch_running_text
    LDX #dispatch_run_label
dispatch_running_text:
    LDAA #120
    CLRB
    JSR paint_text
dispatch_switch_check:
    TST dispatch_hud_force
    BNE dispatch_switch_value
    LDAA dispatch_switch
    CMPA dispatch_old_switch
    BEQ dispatch_next_check
dispatch_switch_value:
    LDAA dispatch_switch
    STAA dispatch_old_switch
    LDX #dispatch_a_label
    TSTA
    BEQ dispatch_switch_text
    LDX #dispatch_b_label
dispatch_switch_text:
    LDAA #174
    LDAB #7
    JSR paint_text
dispatch_next_check:
    TST dispatch_hud_force
    BNE dispatch_next_value
    LDAA dispatch_next_train
    CMPA dispatch_old_next
    BEQ dispatch_reason_check
dispatch_next_value:
    LDAA dispatch_next_train
    STAA dispatch_old_next
    CMPA dispatch_total
    BNE dispatch_next_data
    LDX #dispatch_end_label
    BRA dispatch_next_text
dispatch_next_data:
    LDAB #3
    MUL
    ADDD dispatch_schedule
    ...
    LDAB #4
    MUL
    ADDD #dispatch_route_labels
    XGDX
dispatch_next_text:
    LDAA #138
    LDAB #2
    JSR paint_text
dispatch_reason_check:
    TST dispatch_hud_force
    BNE dispatch_reason_value
    LDAA dispatch_reason
    CMPA dispatch_old_reason
    BEQ dispatch_render_done
dispatch_reason_value:
    LDAA dispatch_reason
    STAA dispatch_old_reason
    TSTA
    BEQ dispatch_render_done
    DECA
    LDAB #6
    MUL
    ADDD #dispatch_reason_labels
    XGDX
    LDAA #138
    LDAB #1
    JMP paint_text
dispatch_render_done:
    RTS
.section .bss, bss
