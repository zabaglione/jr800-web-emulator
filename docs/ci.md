...
| LCDサンプルのソース・生成コード・専用検証コード | そのサンプルのNativeビルド、WASM Debugの短い動作確認、WASM Releaseの全検証 |
| `write-watch`サンプル | NativeのMake・デバッガー操作検証 |
| SDKライブラリ | 全作品・全サンプルとNativeの共通テスト |
| LCDサンプルの共通Make・検証コード | LCDサンプル群。共通メモリー配置変更時はNativeの共通テストも実行 |
| ゲーム共通コード・共通ゲーム検証コード | 全作品 |
...
サンプルは各ディレクトリーのMakefileから個別に検出します。追加時に全ゲームを検証することはありません。LCD一覧のMakefileに名前を追加しただけでも、既存サンプルの検証結果は再利用します。RELIC DIVE GFXは`07-relic-dive`を直接再利用するため、このサンプルを変更した場合はRELIC DIVE GFXも検証します。
...
初回・キャッシュ失効時・CI設定変更時は全体の検証が必要です。成功記録はサンプル別の入力を含むv2形式です。通常の差分実行は、各構成の成功記録ができた次の実行から有効になります。Actionsの各ジョブのSummaryに、本体ビルド・共通テストの有無と対象ゲーム名・サンプル名を表示します。

サンプルのビルド出力は`build/ci-samples/<構成>/<サンプル>/`に分離し、実行前にそのサンプルの出力だけを消去します。成功した入力を記録して再利用し、サンプルの生成物をPagesやキャッシュの配布物には含めません。
