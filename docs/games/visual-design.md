...
RELIC DIVE GFXは元版の状態アドレスと24×16マップを維持するため、専用のメモリー配置を使います。$2800〜$5DFFにコード・状態・1,536バイトの描画面・圧縮タイトル・BASIC復帰領域を収め、$5E00〜$5FFFをスタックとして予約します。実割当13,622バイト、検証時の最大スタック使用14バイトです。共通HUDの数字領域を追加せず、元の文字描画を使った白抜きHUDと新しいタイルで構成しています。
