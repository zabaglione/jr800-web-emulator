...
方向キーで空きマスを動かし、上段1・2・3、中段4・5・6、下段7・8・空きの順に並べます。追加目標は、STAMPに示す数字のタイルを、模様付きの枠のマスへ一度運ぶことです。対象の数字は盤面とSTAMPの両方で白抜き表示になります。タイルは厚みのある枠で描き、盤面のケースと側面のガイドでスライドパズルらしい外観にしています。模様付きの枠は移動先に固定され、達成すると太い枠と BONUS: OK に変わります。最終配置とは別の位置へ立ち寄る必要があります。
...
[プレイ動画を見る（30秒・音声あり）](https://zabaglione.github.io/jr800-web-emulator/videos/#slide-nine)
